...
<div class="container-fluid border-bottom">
    ...
    <div class="container-fluid text-center">
        ...
        <div class="container-fluid text-center col-lg-6 col-sm-12">
            ...
            <c:if test="${(commCheck.c_state=='d' || commCheck.c_state=='e') && idCheck.s_state=='s' || idCheck2.c_id == loginID}">
                <form name="commacForm" id="commacForm" style="margin-bottom: 30px">
                    ...
                    <input type="hidden" id="ac_id" name="ac_id" value="${s_id}"><!-- 후기 작성하는 로그인id -->
            ...
            <!-- 후기 목록 div -->
            <div class="container-fluid">
                <table class="table table-default activityList" id="activityList"></table>
            </div>
...
<script>
    ...
    let loginID = '${s_id}';          //로그인 id가 작성자와 동일한 경우에만 후기 수정/삭제 가능

    //더보기 limit 변수 선언
    let limit = 3;          //3개씩 출력
    let size = ${acCount};  //총 후기 수
    ...
    function listActivity(){
        $.ajax({
             url    :'/comm/aclist'
            ,type   :'get'
            ,data   :{'ac_ccode':ac_ccode, 'limit':limit} //부모 PK키
            ,success:function (data){
                //alert(size);
                alert(limit);
                ...
                $.each(data, function (key, value){
                    // alert(key);
                    // alert(value);
                    a += '<tr class="commacArea" style="border-bottom: 1px solid darkgray; margin-bottom: 15px">';
                    a += '  <td class="commacInfo' + value.ac_no +'">';
                    a += '      후기번호 : ' + value.ac_no + ' / 작성자 : ' + value.ac_id + "  " + value.ac_rdate;
                    a += '  </td><td>';
                    ...
                    a += '  </td>';
                    a += '  </tr><tr>';
                    a += '  <td class="commacReview' + value.ac_no +'">'
