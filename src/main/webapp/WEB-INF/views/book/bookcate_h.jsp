...
<div class="container-fluid" style="text-align: center">
	<!-- endPage는 10, 20, 30.. ex)총 페이지가 22일 때 endPage 30이 아닌 22까지 출력해야 함 -->
        ...
        <c:if test="${startPage > 1}">
            <a href="/book/bookcate_h?pageNum=${startPage-1}">[이전]</a>      <!-- pageNum값을 Controller에 보냄 -->
        </c:if>
        ...
        <c:forEach var="i" begin="${startPage}" end="${endPage}">
            <c:choose>
                ...
                <c:when test="${pageNum != i}"><a href="/book/bookcate_h?pageNum=${i}">[${i}]</a></c:when>
            </c:choose>
        </c:forEach>

        <!-- 다음 이동 -->
        <c:if test="${endPage < pageCount}">
            <a href="/book/bookcate_h?pageNum=${endPage+1}">[다음]</a>
        </c:if>
</div>
